...
<xsl:template match="/">
    <html>
        ...
        <body>
            ...
            <script type="text/javascript">// setTimeout(function() { location.href += ''; }, 1000)</script>
        </body>
    </html>
</xsl:template>
...
<xsl:template match="stream">
    <tr valign="top" class="stream-row">
        ...
        <xsl:attribute name="bgcolor">
            <xsl:choose>
                <xsl:when test="active and (bw_in &gt; 0)">#cccccc</xsl:when>
                <xsl:otherwise>#ffadad</xsl:otherwise>
            </xsl:choose>
        </xsl:attribute>
        <td>
            <a>
